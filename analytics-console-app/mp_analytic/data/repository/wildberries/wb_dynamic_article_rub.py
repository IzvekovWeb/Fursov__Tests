...
class DynamicArticleRubReport:
    """
    Динамика по артикулам (руб)
    """
    ...
    def execute(self):
        for supplier in self.__suppliers:
            ...
            print(f"\rОбрабатываем - {name}")
            self.__get_nomenclatures(supplier['supplier-id'], name)
    ...
    def __get_data(self, supplier_id, supplier_name):
            ...
            for week in brand['weeks']:
                for day in week['days']:
                    ...
                    for orders in day['articles']:
                        article = orders['article'].split('/')[0]
                        ...
                        if self.response.get(article + supplier_name):
                            if self.response.get(article + supplier_name)['days'].get(date):
                                self.response[article + supplier_name]['days'][date] += price
                            else:
                                self.response[article + supplier_name]['days'][date] = price
                        else:
