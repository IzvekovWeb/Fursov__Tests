...
from data.repository.wildberries.wb_dynamic_article_count import DynamicArticleCountReport


class DynamicArticleCountUseCase:
    """
    Отчет "Заказы по артикулам (шт)"
    """

    def __init__(self):
        self.__sheet_title = SHEETS.get('dynamic_art_count').get('title')
        self.__sheet_id = SHEETS.get('dynamic_art_count').get('id')

        self.__table = GoogleSheetAPI(self.__sheet_title, SPREADSHEET_ID, self.__sheet_id)

    def execute(self) -> str:
        response = DynamicArticleCountReport(WB_TOKEN).execute()
...
if __name__ == '__main__':
    DynamicArticleCountUseCase().execute()
